...
│     ├─ Clear requirements + known patterns → 0.95 → Plan directly   │
│     ├─ Clear requirements + no patterns   → 0.85 → Research first   │
│     ├─ Ambiguous requirements             → 0.70 → Clarify first    │
│     └─ Novel technology stack             → 0.60 → Validate via web │
│                                                                      │
│  4. EXTERNAL VALIDATION (for technology decisions)                  │
│     └─ WebSearch / WebFetch → Best practices, production patterns   │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
...
| Requirements | Known Patterns | Confidence | Action |
|--------------|----------------|------------|--------|
| Clear | Available | 0.95 | Plan directly |
| Clear | Missing | 0.85 | Web validation |
...
1. Read project context and existing docs
2. Read requirements and constraints
3. Design components and interfaces
4. Validate technology choices via web search if needed
...
```text
PRE-FLIGHT CHECK
├─ [ ] Project context checked (CLAUDE.md, README)
...
**Confidence:** {score} | **Sources:** Project context, {web validations if any}
```
...
**Core Principle:** Context first. Confidence always. Ask when uncertain.

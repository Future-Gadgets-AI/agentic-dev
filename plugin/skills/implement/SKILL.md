---
name: implement
description: Turns a ready (readiness:ready) issue into code by invoking the environment's SDD (spec-driven development) workflow plugin through its design and build phases, headlessly — synthesizes a minimal design input from the issue's own Definition-of-Ready content, discovers the design/build entrypoints generically at runtime (never hardcodes one plugin's namespace), and returns a parsed build report for the caller's smoke gate and PR body. One atomic skill in the composable-lifecycle architecture (ADR-0009): independently invocable and testable, no sequencing logic of its own — composers own the surrounding order. Use when a composer (`/pickup`, the full-lifecycle engine) needs the "implement" movement of the issue → PR flow, or to smoke-test this movement standalone on one ready issue. Does not re-check readiness, create the branch, or open the PR.
---
...
Reject a candidate that is **interactive-only** — a description reading like "collaborative dialogue" or "capture requirements" (that's brainstorm/define, not design/build), or a tool grant that includes `AskUserQuestion`. Design and build must run unattended.

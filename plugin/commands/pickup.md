...
Run headlessly: never pause to ask permission mid-chain. When a step cannot honestly proceed, escalate asynchronously — label plus bot-attributed comment, per the failure rule in the chain below — and stop cleanly: the issue is the ledger, and a later session resumes from it.
...
- `type:spike` or `type:adr` → refuse: their deliverables (knowledge, a decision) don't fit this branch→PR path; say so. Any other — or missing — `type:` label → refuse: unrecognized type. v1 routes `type:feature`, `type:task`, and built-in `bug` down the one shared path below.
- Missing `readiness:ready` → refuse: the ready label is the autonomy gate (ADR-0004). Point the human at `/refine-issue`.

## The chain

Each step names its owner. Every label write validates against the live label set first (`gh label list`) and never creates a label.

A step that fails or returns Blocked (step 2 onward): escalate asynchronously, per Principle IV of the constitution — label the issue `status:needs-decision`, post the step's report as a bot comment @-mentioning the configured reviewers (`$AGENTIC_REVIEWERS`, available after sourcing bot auth), and stop. The label is what surfaces the card on the board; resuming an escalated card is deferred work (#30). Never improvise recovery past a block, and never round a Blocked result up to done.
...
2. **Phase: in-progress** — move the issue's `phase:` label to `phase:in-progress` (bot).
...
6. **Version bump** — if the repo ships a Claude plugin (a `.claude-plugin/plugin.json` manifest) and shipped plugin files changed, run `${CLAUDE_PLUGIN_ROOT}/scripts/bump.sh --apply` (idempotent; semver level from the branch prefix) and commit the bump.
7. **Open the PR** — load the `create-pr` skill. Draft is the headless default — the human flips it ready. The body must close issue #N (`Closes #N`), carry the logged assumptions from step 1, and embed the `Smoke evidence:` block in its test plan. Its trailing reviewer-request step belongs to step 10 below — skip it there.

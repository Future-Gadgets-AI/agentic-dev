...
Labels are applied directly on the issue (no `Labels:` line in the body). Five namespaces (canonical source: `contracts/labels.md`):
...
| `readiness:` | `draft` · `needs-refinement` · `ready` | Definition-of-Ready state (see `contracts/dor-rubric.md`). Orthogonal to `phase:`. |
